...
"""
A volunteers observation does not come with raster information (associated environmental variables). After being run 
through this function, it will.

@param input_file, contains volunteers observations (does not come with raster information)
@param output_file, the name of our output file. This will contain volunteers observations WITH raster information.
"""
def writeToFile(input_file, output_file):
    ...
    worksheet.write('D1', 'LATITUDEDD_NUM')
    worksheet.write('E1', 'LONGITUDEDD_NUM')
    worksheet.write('F1', 'RECORD_TYPE')
    ...
    for idx, val in enumerate(species):
        ...
        worksheet.write(row, col + 3, lat[idx])
        worksheet.write(row, col + 4, long[idx])
        worksheet.write(row, col + 5, record_type[idx])

        # raster data
        worksheet.write(row, col + 6, calculateRaster(point, yOrigin, xOrigin, veg, pixelWidth, pixelHeight))
        worksheet.write(row, col + 7, calculateRaster(point, yOrigin_wet, xOrigin_wet, wet, pixelWidth_wet, pixelHeight_wet))
        worksheet.write(row, col + 8, calculateRaster(point, yOrigin_summer, xOrigin_summer, summer, pixelWidth_summer, pixelHeight_summer))
        worksheet.write(row, col + 9, calculateRaster(point, yOrigin_summer75, xOrigin_summer75, summer75, pixelWidth_summer75, pixelHeight_summer75))
        worksheet.write(row, col + 10, calculateRaster(point, yOrigin_rain7, xOrigin_rain7, rain7, pixelWidth_rain7, pixelHeight_rain7))
        worksheet.write(row, col + 11, calculateRaster(point, yOrigin_min7, xOrigin_min7, min7, pixelWidth_min7, pixelHeight_min7))
        worksheet.write(row, col + 12, calculateRaster(point, yOrigin_rain1, xOrigin_rain1, rain1, pixelWidth_rain1,
                                   pixelHeight_rain1))
        worksheet.write(row, col + 13, calculateRaster(point, yOrigin_max1, xOrigin_max1, max1, pixelWidth_max1,
                                   pixelHeight_max1))
        worksheet.write(row, col + 14, calculateRaster(point, yOrigin_radioth, xOrigin_radioth, radioth, pixelWidth_radioth,
                                   pixelHeight_radioth))
        worksheet.write(row, col + 15, calculateRaster(point, yOrigin_radiok, xOrigin_radiok, radiok, pixelWidth_radiok,
                                   pixelHeight_radiok))
        worksheet.write(row, col + 16, calculateRaster(point, yOrigin_pi, xOrigin_pi, pi, pixelWidth_pi,
                                   pixelHeight_pi))
        worksheet.write(row, col + 17, calculateRaster(point, yOrigin_vertical, xOrigin_vertical, vertical, pixelWidth_vertical,
                                   pixelHeight_vertical))
        worksheet.write(row, col + 18, calculateRaster(point, yOrigin_land, xOrigin_land, land, pixelWidth_land,
                                   pixelHeight_land))
        worksheet.write(row, col + 19, calculateRaster(point, yOrigin_ibra, xOrigin_ibra, ibra, pixelWidth_ibra,
                                   pixelHeight_ibra))
        worksheet.write(row, col + 20, calculateRaster(point, yOrigin_hydra, xOrigin_hydra, hydra, pixelWidth_hydra,
                                   pixelHeight_hydra))
        worksheet.write(row, col + 21, calculateRaster(point, yOrigin_eco1, xOrigin_eco1, eco1, pixelWidth_eco1,
                                   pixelHeight_eco1))
        worksheet.write(row, col + 22, calculateRaster(point, yOrigin_eco2, xOrigin_eco2, eco2, pixelWidth_eco2,
                                   pixelHeight_eco2))
        worksheet.write(row, col + 23, calculateRaster(point, yOrigin_heat, xOrigin_heat, heat, pixelWidth_heat,
                                   pixelHeight_heat))
        worksheet.write(row, col + 24, calculateRaster(point, yOrigin_streams, xOrigin_streams, streams, pixelWidth_streams,
                                   pixelHeight_streams))
